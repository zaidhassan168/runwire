...
2. An agent requests the same visible flow through WebMCP, and Runwire records every operation in the Agent Trace.
...
Runwire registers 22 page tools through `document.modelContext.registerTool`:
...
| Flow composition | `begin_flow_build`, `add_flow_request`, `set_request_query_parameter`, `set_request_headers`, `set_request_body`, `set_response_extraction`, `run_flow_step` |
...
Every invocation is recorded in the visible Agent Trace. Request bodies, query values, environment values, and tool outputs are excluded from the trace; it shows only safe intent and execution status. Its **Tool calls / API flow** toggle connects `run_journey` and granular `run_flow_step` calls to the actual request sequence, extracted-value edges, response codes, and durations.
...
- Live WebMCP Agent Trace with safe history and an animated tool-to-API execution flow.
...
- Agent-triggered execution remains visible in the Agent Trace, while authentication values and sensitive environment data remain protected.
